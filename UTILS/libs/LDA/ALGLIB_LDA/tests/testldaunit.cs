    ...
    public class testldaunit
    {
                ...
                for (nc = 2; nc <= maxnc; nc++)
                {
                    for (pass = 1; pass <= passcount; pass++)
                    {
                        ...
                        if (nf >= 3)
                        {
                            ...
                            ldanerrors = ldanerrors | !testwn(ref xy, ref wn, ns, nf, nc, 1);
                            ldanerrors = ldanerrors | Math.Sqrt(AP.Math.Sqr(wn[nf - 1, nf - 1]) + AP.Math.Sqr(wn[nf - 2, nf - 1])) <= 0.90;
                            ldanerrors = ldanerrors | Math.Abs(wn[nf - 1, nf - 1] + wn[nf - 2, nf - 1]) >= 0.10;
                            lda.fisherlda(ref xy, ns, nf, nc, ref info, ref w1);
                            for (i = 0; i <= nf - 1; i++)
                            {
                                lda1errors = lda1errors | w1[i] != wn[i, 0];
        ...
        *************************************************************************/
            ...
            int ndeg)
        {
            ...
            {
                result = result & jp[i - 1] / jq[i - 1] >= (1 - tol * AP.Math.MachineEpsilon) * jp[i] / jq[i];
            }
            for (i = nf - 1 - ndeg + 1; i <= nf - 1; i++)
            {
                result = result & jp[i] <= tol * AP.Math.MachineEpsilon * jp[0];
            }
            ...
            for (i = 0; i <= nf - 1; i++)
            {
                wprev = tx[i];
                tx[i] = wprev + 0.01;
                result = result & v >= (1 - tol * AP.Math.MachineEpsilon) * calcj(nf, ref st, ref sw, ref tx, ref p, ref q);
                tx[i] = wprev - 0.01;
                result = result & v >= (1 - tol * AP.Math.MachineEpsilon) * calcj(nf, ref st, ref sw, ref tx, ref p, ref q);
                tx[i] = wprev;
            }
            ...
            if (sevd.smatrixevd(a, nf, 1, true, ref tx, ref z))
            {
                result = result & tx[0] > tx[nf - 1] * 1000 * AP.Math.MachineEpsilon;
            }
            ...
            for (j = 0; j <= nf - 1; j++)
            {
                ...
                v = Math.Sqrt(v);
                result = result & Math.Abs(v - 1) <= 1000 * AP.Math.MachineEpsilon;
                v = 0;
                for (i = 0; i <= nf - 1; i++)
                {
                    v = v + wn[i, j];
