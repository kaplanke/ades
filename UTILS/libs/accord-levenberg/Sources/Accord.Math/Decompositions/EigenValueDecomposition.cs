...
namespace Accord.Math.Decompositions
{
    ...
    public sealed class EigenValueDecomposition
	{
		...
		private Complex[] d; 		// storage of eigenvalues. (d = real, e = imaginary)
		...
		public EigenValueDecomposition(double[,] value)
		{
			...
			n = value.GetLength(1); // get columns
			V = new double[n,n];
			d = new Complex[n];
        ...
        private void tred2()
        {
            ...
            for (int i = n - 1; i > 0; i--)
            {
                ...
                double h = 0.0;
                for (int k = 0; k < i; k++)
                    scale = scale + System.Math.Abs(d[k].Re);
        ...
        private void tql2()
        {
            ...
            for (int l = 0; l < n; l++)
            {
                ...
                tst1 = System.Math.Max(tst1, System.Math.Abs(d[l].Re) + System.Math.Abs(d[l].Im));
                int m = l;
                while (m < n)
                {
                    if (System.Math.Abs(d[m].Im) <= eps * tst1)
                        break;
                    m++;
                }
                ...
                if (m > l)
                {
                    ...
                    do
                    {
                        ...
                        double g = d[l].Re;
                        double p = (d[l + 1].Re - g) / (2.0 * d[l].Im);
                        double r = Math.Tools.Hypotenuse(p, 1.0);
                        if (p < 0)
                        {
                            r = -r;
                        }
                        ...
                        p = -s * s2 * c3 * el1 * d[l].Im / dl1;
                        d[l].Im = s * p;
                        d[l].Re = c * p;

                        // Check for convergence.
                    }
                    while (System.Math.Abs(d[l].Im) > eps * tst1);
                }
                d[l].Re = d[l].Re + f;
                d[l].Im = 0.0;
            }
                    ...
                    for (int i = n - 1; i >= 0; i--)
                    {
                        ...
                        else
                        {
                            ...
                            else
                            {
                                ...
                                x = H[i, i + 1];
                                y = H[i + 1, i];
                                q = (d[i].Re - p) * (d[i].Re - p) + d[i].Im * d[i].Im;
                                t = (x * s - z * r) / q;
                                H[i, n] = t;
                    ...
                    for (int i = n - 2; i >= 0; i--)
                    {
                        ...
                        else
                        {
                            ...
                            else
                            {
                                ...
                                x = H[i, i + 1];
                                y = H[i + 1, i];
                                vr = (d[i].Re - p) * (d[i].Re - p) + d[i].Im * d[i].Im - q * q;
                                vi = (d[i].Re - p) * 2.0 * q;
                                if (vr == 0.0 & vi == 0.0)
